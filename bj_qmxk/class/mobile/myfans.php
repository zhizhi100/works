...
if (true) {
	...
	$fansall = pdo_fetchall("select createtime, from_user from " . tablename('fans') . " fans where from_user!='{$from_user}' and (fans.from_user in (" . $sql1_member . ") ) and fans.weid={$_W['weid']}");
}
if (true && $cfg['globalCommissionLevel'] >= 2) {
	...
	$fansall = pdo_fetchall("	select createtime, from_user from " . tablename('fans') . " fans where from_user!='{$from_user}' and (fans.from_user in (" . $sql2_member . ")) and (fans.from_user not in (" . $sql1_member . ") ) and fans.weid={$_W['weid']}");
}
if (true && $cfg['globalCommissionLevel'] >= 3) {
	...
	$fansall = pdo_fetchall("	select createtime, from_user from " . tablename('fans') . " fans where from_user!='{$from_user}' and (fans.from_user in (" . $sql3_member . ")) and (fans.from_user not in (" . $sql1_member . ")) and (fans.from_user not in  (" . $sql2_member . ")) and fans.weid={$_W['weid']}");
}
