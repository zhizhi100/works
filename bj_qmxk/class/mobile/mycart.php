...
if ($op == 'add') {
    ...
    $goods = pdo_fetch("SELECT id, type, total,marketprice,maxbuy FROM " . tablename('bj_qmxk_goods') . " WHERE id = :id", array(
        ':id' => $goodsid
    ));
    if (empty($goods)) {
    ...
    if (!empty($optionid)) {
        $option = pdo_fetch("select marketprice,stock from " . tablename('bj_qmxk_goods_option') . " where id=:id limit 1", array(
            ":id" => $optionid
        ));
        if (!empty($option)) {
    ...
    $row = pdo_fetch("SELECT id, total FROM " . tablename('bj_qmxk_cart') . " WHERE from_user = :from_user AND weid = '{$_W['weid']}' AND goodsid = :goodsid  and optionid=:optionid", array(
        ':from_user' => $from_user,
        ':goodsid' => $goodsid,
        ':optionid' => $optionid
    ));
    if ($row == false) {
        ...
        pdo_update('bj_qmxk_cart', $data, array(
            'id' => $row['id']
        ));
    }
    ...
    if (!empty($list)) {
        foreach ($list as & $item) {
            $goods = pdo_fetch("SELECT  title, thumb, marketprice, unit, total,maxbuy FROM " . tablename('bj_qmxk_goods') . " WHERE id=:id limit 1", array(
                ":id" => $item['goodsid']
            ));
            $option = pdo_fetch("select title,marketprice,stock from " . tablename("bj_qmxk_goods_option") . " where id=:id limit 1", array(
                ":id" => $item['optionid']
            ));
            if ($option) {
