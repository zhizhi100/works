...
if ($op == 'fansrange') {
    $list = pdo_fetchall("SELECT member.*,(select count(his.sharemid) from " . tablename('bj_qmxk_share_history') . " his where his.sharemid=member.id  ) fanscount FROM " . tablename('bj_qmxk_member') . " member  WHERE member.weid = :weid  order by fanscount desc limit 30", array(
        ':weid' => $_W['weid']
    ));
...
if ($op == 'productsalestatistics') {
    $list = pdo_fetchall("SELECT goods.*,0 as cpersent,(select sum(ordergoods.total) from  " . tablename('bj_qmxk_order_goods') . " ordergoods where ordergoods.goodsid=goods.id and ordergoods.weid=goods.weid ) salescount from " . tablename('bj_qmxk_goods') . " goods  where goods.weid = :weid order by (salescount/goods.viewcount)  desc ", array(
        ':weid' => $_W['weid']
    ));
...
if ($op == 'saledetails') {
    ...
    $list = pdo_fetchall("SELECT ordergoods.price,ordergoods.total,(select title from " . tablename('bj_qmxk_goods') . " goods where ordergoods.goodsid=goods.id) titles,orders.createtime,orders.ordersn from  " . tablename('bj_qmxk_order_goods') . " ordergoods left join " . tablename('bj_qmxk_order') . " orders  on orders.id=ordergoods.orderid where orders.weid = :weid $condition order by orders.createtime  desc  LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':weid' => $_W['weid']
    ));
    $total = pdo_fetchcolumn("SELECT count(ordergoods.id) from  " . tablename('bj_qmxk_order_goods') . " ordergoods left join " . tablename('bj_qmxk_order') . " orders  on orders.id=ordergoods.orderid where orders.weid = :weid $condition order by orders.createtime desc", array(
        ':weid' => $_W['weid']
    ));
