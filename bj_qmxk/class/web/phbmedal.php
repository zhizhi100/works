...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        $data = array(
            ...
            'fans_count' => intval($_GPC['fans_count'])
        );
        if (!empty($id)) {
            pdo_update('bj_qmxk_phb_medal', $data, array(
                'id' => $id,
                'weid' => $_W['weid']
            ));
        } else {
        ...
        message('更新头衔成功！', $this->createWebUrl('phbmedal', array(
            'op' => 'display'
        )) , 'success');
    }
    include $this->template('phbmedal');
    ...
    if (empty($category)) {
        message('抱歉，头衔不存在或是已经被删除！', $this->createWebUrl('phbmedal', array(
            'op' => 'display'
        )) , 'error');
    }
    pdo_delete('bj_qmxk_phb_medal', array(
        'id' => $id,
        'weid' => $_W['weid']
    ));
    message('头衔删除成功！', $this->createWebUrl('phbmedal', array(
        'op' => 'display'
    )) , 'success');
} 
?>