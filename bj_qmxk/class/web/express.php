...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('bj_qmxk_express', $data, array(
                'id' => $id
            ));
        } else {
        ...
        message('更新物流成功！', $this->createWebUrl('express', array(
            'op' => 'display'
        )) , 'success');
    }
    ...
    if (empty($express)) {
        message('抱歉，物流方式不存在或是已经被删除！', $this->createWebUrl('express', array(
            'op' => 'display'
        )) , 'error');
    }
    pdo_delete('bj_qmxk_express', array(
        'id' => $id
    ));
    message('物流方式删除成功！', $this->createWebUrl('express', array(
        'op' => 'display'
    )) , 'success');
} else {
    message('请求方式不存在');
