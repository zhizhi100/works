...
if ($operation == 'display') {
    if (!empty($_GPC['displayorder'])) {
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('bj_qmxk_category', array(
                'displayorder' => $displayorder
            ) , array(
                'id' => $id
            ));
        }
        message('分类排序更新成功！', $this->createWebUrl('category', array(
            'op' => 'display'
        )) , 'success');
    }
    $children = array();
    ...
    if (checksubmit('submit')) {
        ...
        $data = array(
            'weid' => $_W['weid'],
            'cateindexlink' => $_GPC['cateindexlink'],
            'cateindexadvlink' => $_GPC['cateindexadvlink'],
            'name' => $_GPC['catename'],
        ...
        if (!empty($_FILES['cateindexpic']['tmp_name'])) {
           // file_delete($_GPC['thumb_old']);
            $upload = file_upload($_FILES['cateindexpic']);
            if (is_error($upload)) {
                message($upload['message'], '', 'error');
            }
            $data['cateindexpic'] = $upload['path'];
        }
        
        if (!empty($_FILES['cateindexadvpic']['tmp_name'])) {
           // file_delete($_GPC['thumb_old']);
            $upload = file_upload($_FILES['cateindexadvpic']);
            if (is_error($upload)) {
                message($upload['message'], '', 'error');
            }
            $data['cateindexadvpic'] = $upload['path'];
        }
        ...
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('bj_qmxk_category', $data, array(
                'id' => $id,
                'weid' => $_W['weid']
            ));
        } else {
    ...
    pdo_delete('bj_qmxk_category', array(
        'id' => $id,
        'parentid' => $id
    ) , 'OR');
