...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        $data = array(
        	...
        	'pinkage_money' => $_GPC['pinkage_money']
        );
        if (!empty($id)) {
            pdo_update('bj_qmxk_dispatch', $data, array(
                'id' => $id
            ));
        } else {
        ...
        message('更新配送方式成功！', $this->createWebUrl('dispatch', array(
            'op' => 'display'
        )) , 'success');
    }
    ...
    if (empty($dispatch)) {
        message('抱歉，配送方式不存在或是已经被删除！', $this->createWebUrl('dispatch', array(
            'op' => 'display'
        )) , 'error');
    }
    pdo_delete('bj_qmxk_dispatch', array(
        'id' => $id
    ));
    message('配送方式删除成功！', $this->createWebUrl('dispatch', array(
        'op' => 'display'
    )) , 'success');
} else {
    message('请求方式不存在');
