...
if ($operation == 'post') {
    ...
    if (!empty($award_id)) {
        $item = pdo_fetch("SELECT * FROM " . tablename('bj_qmxk_credit_award') . " WHERE award_id = :award_id", array(
            ':award_id' => $award_id
        ));
        if (empty($item)) {
    ...
    if (checksubmit('submit')) {
        ...
        message('商品更新成功！', create_url('site/module/award', array(
            'name' => 'bj_qmxk',
            'op' => 'display'
        )) , 'success');
    }
    ...
    $award_id = intval($_GPC['award_id']);
    $row = pdo_fetch("SELECT award_id FROM " . tablename('bj_qmxk_credit_award') . " WHERE award_id = :award_id", array(
        ':award_id' => $award_id
    ));
    if (empty($row)) {
        message('抱歉，商品' . $award_id . '不存在或是已经被删除！');
    }
    pdo_delete('bj_qmxk_credit_award', array(
        'award_id' => $award_id
    ));
