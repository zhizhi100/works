...
if ($operation == 'post') {
    $id = intval($_GPC['id']);
    if (!empty($id)) {
        $item = pdo_fetch("SELECT * FROM " . tablename('bj_qmxk_goods') . " WHERE id = :id", array(
            ':id' => $id
        ));
        if (empty($item)) {
        ...
        foreach ($allspecs as & $s) {
            $s['items'] = pdo_fetchall("select * from " . tablename('bj_qmxk_spec_item') . " where specid=:specid order by displayorder asc", array(
                ":specid" => $s['id']
            ));
        }
        unset($s);
        $params = pdo_fetchall("select * from " . tablename('bj_qmxk_goods_param') . " where goodsid=:id order by displayorder asc", array(
            ':id' => $id
        ));
        $piclist = unserialize($item['thumb_url']);
        $html = "";
        $options = pdo_fetchall("select * from " . tablename('bj_qmxk_goods_option') . " where goodsid=:id order by id asc", array(
            ':id' => $id
        ));
        $specs = array();
        if (count($options) > 0) {
            ...
            $html.= '<th><div class="input-append input-prepend"><span class="add-on">积分</span><input type="text" class="span1 option_profit_all"  VALUE=""/><span class="add-on"><a href="javascript:;" class="icon-hand-down" title="批量设置" onclick="setCol(\'option_profit\');"></a></span></div></th>';
            ...
            for ($i = 0; $i < $newlen; $i++) {
                ...
                $hh.='<td><input name="option_profit_' . $ids . '[]" type="text" class="span1 option_profit option_profit_' . $ids . '" " value="' . $val['profit'] . '"/></td>';
    ...
    if (empty($category)) {
        message('抱歉，请您先添加商品分类！', $this->createWebUrl('category', array(
            'op' => 'post'
        )) , 'error');
    }
    if (checksubmit('submit')) {
        ...
        $data = array(
        	...
        	'isrecommandcategory'=> intval($_GPC['isrecommandcategory']) ,
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (!is_numeric($get_param_id)) {
                ...
                pdo_update("bj_qmxk_goods_param", $a, array(
                    'id' => $get_param_id
                ));
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            if (is_numeric($get_spec_id)) {
                pdo_update("bj_qmxk_spec", $a, array(
                    "id" => $get_spec_id
                ));
                $spec_id = $get_spec_id;
            } else {
            ...
            for ($n = 0; $n < $itemlen; $n++) {
				...
				$old = $_POST["spec_item_oldthumb_" . $get_item_id];
                ...
                if (is_numeric($get_item_id)) {
                    pdo_update("bj_qmxk_spec_item", $d, array(
                        "id" => $get_item_id
                    ));
                    $item_id = $get_item_id;
                } else {
		...
		$option_profit = $_POST['option_profit'];
        ...
        for ($k = 0; $k < $len; $k++) {
            ...
            $a = array(
				...
				"profit" => $_GPC['option_profit_' . $ids][0],
            ...
            if (empty($get_option_id)) {
                ...
                pdo_update("bj_qmxk_goods_option", $a, array(
                    'id' => $get_option_id
                ));
    ...
    if($_GPC['up_shelves'] && is_array($_GPC['chk'])){
        foreach ($_GPC['chk'] as $key => $v) {
            pdo_update("bj_qmxk_goods", array("status" => 1),array("id" => $v));
        }
        message('上架成功！', referer() , 'success');
    }else if($_GPC['down_shelves'] && is_array($_GPC['chk'])){
        foreach ($_GPC['chk'] as $key => $v) {
            pdo_update("bj_qmxk_goods", array("status" => -1),array("id" => $v));
        }
        message('下架成功！', referer() , 'success');
    }
    ...
    if (!empty($_GPC['goods_id'])) {
        $condition.= " AND id = '{$_GPC['goods_id']}'";
    }
    if (!empty($_GPC['goodssn'])) {
        $condition.= " AND goodssn LIKE '%{$_GPC['goodssn']}%'";
    }
    ...
    }elseif(!empty($_GPC['start_time'])){
        $condition.= " and createtime>=" . $start_time;
    }elseif(!empty($_GPC['end_time'])){
    ...
    $id = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id, thumb FROM " . tablename('bj_qmxk_goods') . " WHERE id = :id", array(
        ':id' => $id
    ));
    if (empty($row)) {
    ...
    $id = intval($_GPC['id']);
    pdo_delete('bj_qmxk_product', array(
        'id' => $id
    ));
