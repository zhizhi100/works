...
$rulekeywordcount = pdo_fetchcolumn("SELECT count(id) FROM " . tablename('rule_keyword') . " WHERE weid=:weid and module='bj_qmxk' and content='分销专属二维码'", array(
    ":weid" => $_W['weid']
));
$boolrule = false;
...
if ($op == 'checkspreadrule') {
    if (!empty($_GPC['boolrule'])) {
        $rulekeywordcount = pdo_fetchall("SELECT rid FROM " . tablename('rule_keyword') . " WHERE weid=:weid and module='bj_qmxk' and content='分销专属二维码'", array(
            ":weid" => $_W['weid']
        ));
...
if ($op == 'delete') {
    pdo_update('bj_qmxk_channel', array(
        'isdel' => 1,
        'createtime' => time()
    ) , array(
    ...
    $mylist = pdo_fetchall("SELECT * FROM " . tablename('bj_qmxk_channel') . " WHERE weid=:weid and isdel=0", array(
        ":weid" => $_W['weid']
    ));
} else if ($op == 'active') {
    ...
    if (!empty($_GPC['channel'])) {
        $item = pdo_fetch("SELECT * FROM " . tablename('bj_qmxk_channel') . " WHERE weid=:weid AND channel=:channel", array(
            ":weid" => $_W['weid'],
            ":channel" => $_GPC['channel']
        ));
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($_GPC['channel'])) {
            ...
            $list_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('bj_qmxk_channel') . " WHERE weid=:weid and isdel=0", array(
                ":weid" => $_W['weid']
            ));
            $active = ($list_count == 0);
            pdo_insert('bj_qmxk_channel', array(
                'title' => $_GPC['title'],
                'createtime' => time() ,
                'notice' => $_GPC['notice'],
                'msgtype' => $_GPC['msgtype'],
            ...
            message('新建成功', $this->createWebUrl('spread', array(
                'op' => 'leaflet'
            )) , 'success');
        }
    }
...
UNION (select m.from_user from " . tablename('bj_qmxk_member') . " m where m.weid = :weid)  )   group by a.from_user  ORDER BY follower_count DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}";
    $mylist = pdo_fetchall($my_follows_sql, array(
        ':weid' => $_W['weid']
    ));
    if (!empty($mylist)) {
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('fans') . " a WHERE a.weid=:weid ", array(
            ':weid' => $_W['weid']
        ));
    ...
    $mylist = pdo_fetchall("SELECT b.createtime createtime, nickname, avatar FROM " . tablename('bj_qmxk_share_history') . " a LEFT JOIN " . tablename('fans') . " b ON a.weid=b.weid and a.from_user = b.from_user WHERE a.sharemid = (select id from " . tablename('bj_qmxk_member') . " c where c.from_user=:leader and c.weid=:weid  limit 1) and a.from_user!=:leader AND a.weid=:weid  ", array(
        ':leader' => $from_user,
        ':weid' => $_W['weid']
    ));
} else {
