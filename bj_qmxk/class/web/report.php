...
$objPHPExcel->getProperties()->setCreator("朗慧科技")->setLastModifiedBy("朗慧科技")->setTitle("Office 2007 XLSX Test Document")->setSubject("Office 2007 XLSX Test Document")->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")->setKeywords("office 2007 openxml php")->setCategory("report file");
...
if ($report == 'orderstatistics') {
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '订单号')->setCellValue('B1', '下单时间')->setCellValue('C1', '总订单金额')->setCellValue('D1', '付款方式')->setCellValue('E1', '用户名')->setCellValue('F1', '收货人');
    $i = 2;
    $index = 0;
    $countmoney = 0;
    foreach ($list as $item) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $i, $item['ordersn'])->setCellValue('B' . $i, date('Y-m-d  H:i:s', $item['createtime']))->setCellValue('C' . $i, $item['price'] . $priceother)->setCellValue('D' . $i, $paytype)->setCellValue('E' . $i, $item['realnamestr'])->setCellValue('F' . $i, $item['tdrealname']);
        $i++;
        foreach ($item['ordergoods'] as $itemgoods) {
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A' . $i, "")->setCellValue('B' . $i, "名称：" . $itemgoods['title'])->setCellValue('C' . $i, "规格：" . $itemgoods['optionname'])->setCellValue('D' . $i, "商品单价：" . $itemgoods['price'])->setCellValue('E' . $i, "购买数量：" . $itemgoods['total'])->setCellValue('F' . $i, "总价(元)：" . round(($itemgoods['total'] * $itemgoods['price']) , 2));
...
if ($report == 'order') {
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '订单编号')->setCellValue('B1', '商品标题')
    ->setCellValue('C1', '商品规格')->setCellValue('D1', '商品编号')
    ->setCellValue('E1', '商品条码')->setCellValue('F1', '数量')->setCellValue('G1', '收货人姓名')
    ->setCellValue('H1', '	联系电话')->setCellValue('I1', '收货地址')->setCellValue('J1', '下单时间');
    $i = 2;
    foreach ($order_list as $item) {
        $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A' . $i, $item['ordersn'],PHPExcel_Cell_DataType::TYPE_STRING)
        ->setCellValue('B' . $i, $item['title'])->setCellValue('C' . $i, $item['optionname'])
        ->setCellValue('D' . $i, $item['goodssn'])->setCellValue('E' . $i, $item['productsn'])
        ->setCellValue('F' . $i, $item['total'])->setCellValue('G' . $i, $item['realname'])
        ->setCellValue('H' . $i, $item['mobile'])
        ->setCellValue('I' . $i, $item['province']."".$item['city']."".$item['area']."".$item['address'])
        ->setCellValue('J' . $i, date('Y-m-d  H:i:s', $item['createtime']));
        $i++;
    }
